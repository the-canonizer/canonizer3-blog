...
class Nodeps {
	const API_HOST = 'rest.Nodeps.com';
	const API_PORT = 80;
	...
	private static function init_hooks() {
		...
		add_action( 'wp_insert_comment', array( 'Nodeps', 'auto_check_update_meta' ), 10, 2 );
		add_filter( 'preprocess_comment', array( 'Nodeps', 'auto_check_comment' ), 1 );
		add_filter( 'rest_pre_insert_comment', array( 'Nodeps', 'rest_auto_check_comment' ), 1 );

		add_action( 'Nodeps_scheduled_delete', array( 'Nodeps', 'delete_old_comments' ) );
		add_action( 'Nodeps_scheduled_delete', array( 'Nodeps', 'delete_old_comments_meta' ) );
		add_action( 'Nodeps_scheduled_delete', array( 'Nodeps', 'delete_orphaned_commentmeta' ) );
		add_action( 'Nodeps_schedule_cron_recheck', array( 'Nodeps', 'cron_recheck' ) );

		add_action( 'comment_form',  array( 'Nodeps',  'add_comment_nonce' ), 1 );

		add_action( 'admin_head-edit-comments.php', array( 'Nodeps', 'load_form_js' ) );
		add_action( 'comment_form', array( 'Nodeps', 'load_form_js' ) );
		add_action( 'comment_form', array( 'Nodeps', 'inject_ak_js' ) );
		add_filter( 'script_loader_tag', array( 'Nodeps', 'set_form_js_async' ), 10, 3 );

		add_filter( 'comment_moderation_recipients', array( 'Nodeps', 'disable_moderation_emails_if_unreachable' ), 1000, 2 );
		add_filter( 'pre_comment_approved', array( 'Nodeps', 'last_comment_status' ), 10, 2 );
		
		add_action( 'transition_comment_status', array( 'Nodeps', 'transition_comment_status' ), 10, 3 );

		// Run this early in the pingback call, before doing a remote fetch of the source uri
		add_action( 'xmlrpc_call', array( 'Nodeps', 'pre_check_pingback' ) );
		
		// Jetpack compatibility
		add_filter( 'jetpack_options_whitelist', array( 'Nodeps', 'add_to_jetpack_options_whitelist' ) );
		add_action( 'update_option_wordpress_api_key', array( 'Nodeps', 'updated_option' ), 10, 2 );
		add_action( 'add_option_wordpress_api_key', array( 'Nodeps', 'added_option' ), 10, 2 );

		add_action( 'comment_form_after',  array( 'Nodeps',  'display_comment_form_privacy_notice' ) );
	}

	public static function get_api_key() {
		return apply_filters( 'Nodeps_get_api_key', defined('WPCOM_API_KEY') ? constant('WPCOM_API_KEY') : get_option('wordpress_api_key') );
	}

	public static function check_key_status( $key, $ip = null ) {
		return self::http_post( Nodeps::build_query( array( 'key' => $key, 'blog' => get_option( 'home' ) ) ), 'verify-key', $ip );
	}
	...
	public static function deactivate_key( $key ) {
		$response = self::http_post( Nodeps::build_query( array( 'key' => $key, 'blog' => get_option( 'home' ) ) ), 'deactivate' );
	...
	/**
	 * Add the Nodeps option to the Jetpack options management whitelist.
	 *
	...
	/**
	 * When the Nodeps option is updated, run the registration call.
	 *
	...
	public static function auto_check_comment( $commentdata ) {
		...
		if ( ! empty( $comment['user_ID'] ) ) {
			$comment['user_role'] = Nodeps::get_user_roles( $comment['user_ID'] );
		}

		/** See filter documentation in init_hooks(). */
		$Nodeps_nonce_option = apply_filters( 'Nodeps_comment_nonce', get_option( 'Nodeps_comment_nonce' ) );
		$comment['Nodeps_comment_nonce'] = 'inactive';
		if ( $Nodeps_nonce_option == 'true' || $Nodeps_nonce_option == '' ) {
			$comment['Nodeps_comment_nonce'] = 'failed';
			if ( isset( $_POST['Nodeps_comment_nonce'] ) && wp_verify_nonce( $_POST['Nodeps_comment_nonce'], 'Nodeps_comment_nonce_' . $comment['comment_post_ID'] ) )
				$comment['Nodeps_comment_nonce'] = 'passed';
		...
		$response = self::http_post( Nodeps::build_query( $comment ), 'comment-check' );

		do_action( 'Nodeps_comment_check_response', $response );

		$commentdata['comment_as_submitted'] = array_intersect_key( $comment, self::$comment_as_submitted_allowed_keys );
		$commentdata['Nodeps_result']       = $response[1];

		if ( isset( $response[0]['x-Nodeps-pro-tip'] ) )
	        $commentdata['Nodeps_pro_tip'] = $response[0]['x-Nodeps-pro-tip'];

		if ( isset( $response[0]['x-Nodeps-error'] ) ) {
			// An error occurred that we anticipated (like a suspended key) and want the user to act on.
			// Send to moderation.
			self::$last_comment_result = '0';
		}
		else if ( 'true' == $response[1] ) {
			// Nodeps_spam_count will be incremented later by comment_is_spam()
			self::$last_comment_result = 'spam';

			$discard = ( isset( $commentdata['Nodeps_pro_tip'] ) && $commentdata['Nodeps_pro_tip'] === 'discard' && self::allow_discard() );

			do_action( 'Nodeps_spam_caught', $discard );

			if ( $discard ) {
				// The spam is obvious, so we're bailing out early. 
				// Nodeps_result_spam() won't be called so bump the counter here
				if ( $incr = apply_filters( 'Nodeps_spam_count_incr', 1 ) ) {
					update_option( 'Nodeps_spam_count', get_option( 'Nodeps_spam_count' ) + $incr );
				}

				if ( self::$is_rest_api_call ) {
					return new WP_Error( 'Nodeps_rest_comment_discarded', __( 'Comment discarded.', 'Nodeps' ) );
				}
				else {
		...
		if ( 'true' != $response[1] && 'false' != $response[1] ) {
			...
			if ( ! wp_next_scheduled( 'Nodeps_schedule_cron_recheck' ) ) {
				wp_schedule_single_event( time() + 1200, 'Nodeps_schedule_cron_recheck' );
				do_action( 'Nodeps_scheduled_recheck', 'invalid-response-' . $response[1] );
			}
		...
		if ( ! wp_next_scheduled( 'Nodeps_scheduled_delete' ) ) {
			wp_schedule_event( time(), 'daily', 'Nodeps_scheduled_delete' );
		}
	...
	public static function auto_check_update_meta( $id, $comment ) {
		...
		if ( is_object( $comment ) && !empty( self::$last_comment ) && is_array( self::$last_comment ) ) {
			if ( self::matches_last_comment( $comment ) ) {
					...
					load_plugin_textdomain( 'Nodeps' );
					
					// normal result: true or false
					if ( self::$last_comment['Nodeps_result'] == 'true' ) {
						update_comment_meta( $comment->comment_ID, 'Nodeps_result', 'true' );
					...
					elseif ( self::$last_comment['Nodeps_result'] == 'false' ) {
						update_comment_meta( $comment->comment_ID, 'Nodeps_result', 'false' );
						...
						update_comment_meta( $comment->comment_ID, 'Nodeps_error', time() );
						self::update_comment_history(
							$comment->comment_ID,
							'',
							'check-error',
							array( 'response' => substr( self::$last_comment['Nodeps_result'], 0, 50 ) )
						);
					}

					// record the complete original data as submitted for checking
					if ( isset( self::$last_comment['comment_as_submitted'] ) )
						update_comment_meta( $comment->comment_ID, 'Nodeps_as_submitted', self::$last_comment['comment_as_submitted'] );

					if ( isset( self::$last_comment['Nodeps_pro_tip'] ) )
						update_comment_meta( $comment->comment_ID, 'Nodeps_pro_tip', self::$last_comment['Nodeps_pro_tip'] );
			}
		}
	}
	...
	public static function delete_old_comments() {
		 ...
		 *
		 * @param int The default, as defined by Nodeps_DELETE_LIMIT.
		 */
		$delete_limit = apply_filters( 'Nodeps_delete_comment_limit', defined( 'Nodeps_DELETE_LIMIT' ) ? Nodeps_DELETE_LIMIT : 10000 );
		 ...
		 */
		$delete_interval = apply_filters( 'Nodeps_delete_comment_interval', 15 );
		...
		while ( $comment_ids = $wpdb->get_col( $wpdb->prepare( "SELECT comment_id FROM {$wpdb->comments} WHERE DATE_SUB(NOW(), INTERVAL %d DAY) > comment_date_gmt AND comment_approved = 'spam' LIMIT %d", $delete_interval, $delete_limit ) ) ) {
			...
			foreach ( $comment_ids as $comment_id ) {
				...
				do_action( 'Nodeps_batch_delete_count', __FUNCTION__ );
			}
			...
			do_action( 'Nodeps_delete_comment_batch', count( $comment_ids ) );
		}

		if ( apply_filters( 'Nodeps_optimize_table', ( mt_rand(1, 5000) == 11), $wpdb->comments ) ) // lucky number
	...
	public static function delete_old_comments_meta() {
		...
		$interval = apply_filters( 'Nodeps_delete_commentmeta_interval', 15 );
		...
		// Nodeps_as_submitted meta values are large, so expire them
		// after $interval days regardless of the comment status
		while ( $comment_ids = $wpdb->get_col( $wpdb->prepare( "SELECT m.comment_id FROM {$wpdb->commentmeta} as m INNER JOIN {$wpdb->comments} as c USING(comment_id) WHERE m.meta_key = 'Nodeps_as_submitted' AND DATE_SUB(NOW(), INTERVAL %d DAY) > c.comment_date_gmt LIMIT 10000", $interval ) ) ) {
			if ( empty( $comment_ids ) )
				return;
			...
			foreach ( $comment_ids as $comment_id ) {
				delete_comment_meta( $comment_id, 'Nodeps_as_submitted' );
				do_action( 'Nodeps_batch_delete_count', __FUNCTION__ );
			}

			do_action( 'Nodeps_delete_commentmeta_batch', count( $comment_ids ) );
		}

		if ( apply_filters( 'Nodeps_optimize_table', ( mt_rand(1, 5000) == 11), $wpdb->commentmeta ) ) // lucky number
	...
	public static function delete_orphaned_commentmeta() {
		...
		while ( $commentmeta_results = $wpdb->get_results( $wpdb->prepare( "SELECT m.meta_id, m.comment_id, m.meta_key FROM {$wpdb->commentmeta} as m LEFT JOIN {$wpdb->comments} as c USING(comment_id) WHERE c.comment_id IS NULL AND m.meta_id > %d ORDER BY m.meta_id LIMIT 1000", $last_meta_id ) ) ) {
			...
			foreach ( $commentmeta_results as $commentmeta ) {
				if ( 'Nodeps_' == substr( $commentmeta->meta_key, 0, 8 ) ) {
			...
			do_action( 'Nodeps_delete_commentmeta_batch', $commentmeta_deleted );
	...
	public static function get_comment_history( $comment_id ) {
		$history = get_comment_meta( $comment_id, 'Nodeps_history', false );
		usort( $history, array( 'Nodeps', '_cmp_time' ) );
		return $history;
	}
	...
	public static function update_comment_history( $comment_id, $message, $event=null, $meta=null ) {
		...
		$r = add_comment_meta( $comment_id, 'Nodeps_history', $event, false );
	}
	...
	public static function check_db_comment( $id, $recheck_reason = 'recheck_queue' ) {
		...
		if ( ! $c ) {
			return new WP_Error( 'invalid-comment-id', __( 'Comment not found.', 'Nodeps' ) );
		}
		...
		$c['user_role'] = '';
		if ( ! empty( $c['user_ID'] ) ) {
			$c['user_role'] = Nodeps::get_user_roles( $c['user_ID'] );
		}

		if ( self::is_test_mode() )
			$c['is_test'] = 'true';

		$response = self::http_post( Nodeps::build_query( $c ), 'comment-check' );
	...
	public static function recheck_comment( $id, $recheck_reason = 'recheck_queue' ) {
		add_comment_meta( $id, 'Nodeps_rechecking', true );
		
		$api_response = self::check_db_comment( $id, $recheck_reason );

		delete_comment_meta( $id, 'Nodeps_rechecking' );
		...
		if ( is_wp_error( $api_response ) ) {
			...
			update_comment_meta( $id, 'Nodeps_result', 'true' );
			delete_comment_meta( $id, 'Nodeps_error' );
			delete_comment_meta( $id, 'Nodeps_delayed_moderation_email' );
			Nodeps::update_comment_history( $id, '', 'recheck-spam' );
		}
		elseif ( 'false' === $api_response ) {
			update_comment_meta( $id, 'Nodeps_result', 'false' );
			delete_comment_meta( $id, 'Nodeps_error' );
			delete_comment_meta( $id, 'Nodeps_delayed_moderation_email' );
			Nodeps::update_comment_history( $id, '', 'recheck-ham' );
		}
		else {
			// abnormal result: error
			update_comment_meta( $id, 'Nodeps_result', 'error' );
			Nodeps::update_comment_history(
				$id,
				'',
				'recheck-error',
	...
	public static function transition_comment_status( $new_status, $old_status, $comment ) {
		...
		if ( 'spam' === $new_status || 'spam' === $old_status ) {
			...
			wp_cache_delete( 'Nodeps_spam_count', 'widget' );
		}
		...
		if ( get_comment_meta( $comment->comment_ID, 'Nodeps_rechecking' ) )
			return;
		
		// Assumption alert:
		// We want to submit comments to Nodeps only when a moderator explicitly spams or approves it - not if the status
	...
	public static function submit_spam_comment( $comment_id ) {
		...
		$as_submitted = self::sanitize_comment_as_submitted( get_comment_meta( $comment_id, 'Nodeps_as_submitted', true ) );
		...
		$response = Nodeps::http_post( Nodeps::build_query( $comment ), 'submit-spam' );
		if ( $comment->reporter ) {
			self::update_comment_history( $comment_id, '', 'report-spam' );
			update_comment_meta( $comment_id, 'Nodeps_user_result', 'true' );
			update_comment_meta( $comment_id, 'Nodeps_user', $comment->reporter );
		}

		do_action('Nodeps_submit_spam_comment', $comment_id, $response[1]);
	}
	...
	public static function submit_nonspam_comment( $comment_id ) {
		...
		$response = self::http_post( Nodeps::build_query( $comment ), 'submit-ham' );
		if ( $comment->reporter ) {
			self::update_comment_history( $comment_id, '', 'report-ham' );
			update_comment_meta( $comment_id, 'Nodeps_user_result', 'false' );
			update_comment_meta( $comment_id, 'Nodeps_user', $comment->reporter );
		}

		do_action('Nodeps_submit_nonspam_comment', $comment_id, $response[1]);
	}
	...
	public static function cron_recheck() {
		...
		if ( get_option( 'Nodeps_alert_code' ) || $status == 'invalid' ) {
			// since there is currently a problem with the key, reschedule a check for 6 hours hence
			wp_schedule_single_event( time() + 21600, 'Nodeps_schedule_cron_recheck' );
			do_action( 'Nodeps_scheduled_recheck', 'key-problem-' . get_option( 'Nodeps_alert_code' ) . '-' . $status );
			return false;
		}

		delete_option('Nodeps_available_servers');

		$comment_errors = $wpdb->get_col( "SELECT comment_id FROM {$wpdb->commentmeta} WHERE meta_key = 'Nodeps_error'	LIMIT 100" );
		...
		foreach ( (array) $comment_errors as $comment_id ) {
			...
			if (
				...
				delete_comment_meta( $comment_id, 'Nodeps_error' );
				delete_comment_meta( $comment_id, 'Nodeps_delayed_moderation_email' );
				continue;
			}

			add_comment_meta( $comment_id, 'Nodeps_rechecking', true );
			...
			if ( !empty( $event ) ) {
				...
				update_comment_meta( $comment_id, 'Nodeps_result', $status );
				...
				if ( $comment && 'unapproved' == wp_get_comment_status( $comment_id ) ) {
					if ( $status == 'true' ) {
						...
						else if ( get_comment_meta( $comment_id, 'Nodeps_delayed_moderation_email', true ) )
				...
				delete_comment_meta( $comment_id, 'Nodeps_rechecking' );
				wp_schedule_single_event( time() + 1200, 'Nodeps_schedule_cron_recheck' );
				do_action( 'Nodeps_scheduled_recheck', 'check-db-comment-' . $status );
				return;
			}
			delete_comment_meta( $comment_id, 'Nodeps_rechecking' );
		}

		$remaining = $wpdb->get_var( "SELECT COUNT(*) FROM {$wpdb->commentmeta} WHERE meta_key = 'Nodeps_error'" );
		if ( $remaining && !wp_next_scheduled('Nodeps_schedule_cron_recheck') ) {
			wp_schedule_single_event( time() + 1200, 'Nodeps_schedule_cron_recheck' );
			do_action( 'Nodeps_scheduled_recheck', 'remaining' );
		}
	}

	public static function fix_scheduled_recheck() {
		$future_check = wp_next_scheduled( 'Nodeps_schedule_cron_recheck' );
		if ( !$future_check ) {
			return;
		}

		if ( get_option( 'Nodeps_alert_code' ) > 0 ) {
			return;
		}

		$check_range = time() + 1200;
		if ( $future_check > $check_range ) {
			wp_clear_scheduled_hook( 'Nodeps_schedule_cron_recheck' );
			wp_schedule_single_event( time() + 300, 'Nodeps_schedule_cron_recheck' );
			do_action( 'Nodeps_scheduled_recheck', 'fix-scheduled-recheck' );
		}
	}

	public static function add_comment_nonce( $post_id ) {
		/**
		 * To disable the Nodeps comment nonce, add a filter for the 'Nodeps_comment_nonce' tag
		 * and return any string value that is not 'true' or '' (empty string).
		 *
		 * Don't return boolean false, because that implies that the 'Nodeps_comment_nonce' option
		 * has not been set and that Nodeps should just choose the default behavior for that
		 * situation.
		 */
		$Nodeps_comment_nonce_option = apply_filters( 'Nodeps_comment_nonce', get_option( 'Nodeps_comment_nonce' ) );

		if ( $Nodeps_comment_nonce_option == 'true' || $Nodeps_comment_nonce_option == '' ) {
			echo '<p style="display: none;">';
			wp_nonce_field( 'Nodeps_comment_nonce_' . $post_id, 'Nodeps_comment_nonce', FALSE );
			echo '</p>';
		}
	}

	public static function is_test_mode() {
		return defined('Nodeps_TEST_MODE') && Nodeps_TEST_MODE;
	}
	...
	public static function allow_discard() {
		...
		return ( get_option( 'Nodeps_strictness' ) === '1'  );
	}
	...
	/**
	 * If Nodeps is temporarily unreachable, we don't want to "spam" the blogger with
		...
		if ( ! empty( self::$prevent_moderation_email_for_these_comments ) && ! empty( $emails ) ) {
			...
			foreach ( self::$prevent_moderation_email_for_these_comments as $possible_match ) {
				if ( self::comments_match( $possible_match, $comment ) ) {
					update_comment_meta( $comment_id, 'Nodeps_delayed_moderation_email', true );
					return array();
				}
			}
		}
	...
	/**
	 * Make a POST request to the Nodeps API.
	 *
	...
	public static function http_post( $request, $path, $ip=null ) {
		...
		$Nodeps_ua = sprintf( 'WordPress/%s | Nodeps/%s', $GLOBALS['wp_version'], constant( 'Nodeps_VERSION' ) );
		$Nodeps_ua = apply_filters( 'Nodeps_ua', $Nodeps_ua );
		...
		// needed by Nodeps_Admin::check_server_connectivity()
		...
		$http_args = array(
			...
			'headers' => array(
				...
				'Host' => $host,
				'User-Agent' => $Nodeps_ua,
			),
			'httpversion' => '1.0',
			'timeout' => 15
		);

		$Nodeps_url = $http_Nodeps_url = "http://{$http_host}/1.1/{$path}";
		...
		$ssl_disabled = get_option( 'Nodeps_ssl_disabled' );

		if ( $ssl_disabled && $ssl_disabled < ( time() - 60 * 60 * 24 ) ) { // 24 hours
			$ssl_disabled = false;
			delete_option( 'Nodeps_ssl_disabled' );
		}
		else if ( $ssl_disabled ) {
			do_action( 'Nodeps_ssl_disabled' );
		}

		if ( ! $ssl_disabled && ( $ssl = wp_http_supports( array( 'ssl' ) ) ) ) {
			$Nodeps_url = set_url_scheme( $Nodeps_url, 'https' );

			do_action( 'Nodeps_https_request_pre' );
		}

		$response = wp_remote_post( $Nodeps_url, $http_args );

		Nodeps::log( compact( 'Nodeps_url', 'http_args', 'response' ) );

		if ( $ssl && is_wp_error( $response ) ) {
			do_action( 'Nodeps_https_request_failure', $response );
			...
			if ( is_wp_error( $response ) ) {
				...
				do_action( 'Nodeps_http_request_pre' );

				// Try the request again without SSL.
				$response = wp_remote_post( $http_Nodeps_url, $http_args );

				Nodeps::log( compact( 'http_Nodeps_url', 'http_args', 'response' ) );
			}
		}

		if ( is_wp_error( $response ) ) {
			do_action( 'Nodeps_request_failure', $response );
		...
		if ( $ssl_failed ) {
			...
			update_option( 'Nodeps_ssl_disabled', time() );
			
			do_action( 'Nodeps_https_disabled' );
		}
	...
	public static function update_alert( $response ) {
		$code = $msg = null;
		if ( isset( $response[0]['x-Nodeps-alert-code'] ) ) {
			$code = $response[0]['x-Nodeps-alert-code'];
			$msg  = $response[0]['x-Nodeps-alert-msg'];
		}

		// only call update_option() if the value has changed
		if ( $code != get_option( 'Nodeps_alert_code' ) ) {
			if ( ! $code ) {
				delete_option( 'Nodeps_alert_code' );
				delete_option( 'Nodeps_alert_msg' );
			}
			else {
				update_option( 'Nodeps_alert_code', $code );
				update_option( 'Nodeps_alert_msg', $msg );
			}
		}
	}
	...
	public static function load_form_js() {
		...
		wp_register_script( 'Nodeps-form', plugin_dir_url( __FILE__ ) . '_inc/form.js', array(), Nodeps_VERSION, true );
		wp_enqueue_script( 'Nodeps-form' );
	}
...
<?php
		if ( $deactivate ) {
			...
			$Nodeps = plugin_basename( Nodeps__PLUGIN_DIR . 'Nodeps.php' );
			$update  = false;
	...
	public static function view( $name, array $args = array() ) {
		$args = apply_filters( 'Nodeps_view_arguments', $args, $name );
		...
		$file = Nodeps__PLUGIN_DIR . 'views/'. $name . '.php';
	...
	public static function plugin_activation() {
		if ( version_compare( $GLOBALS['wp_version'], Nodeps__MINIMUM_WP_VERSION, '<' ) ) {
			load_plugin_textdomain( 'Nodeps' );
			
			$message = '<strong>'.sprintf(esc_html__( 'Nodeps %s requires WordPress %s or higher.' , 'Nodeps'), Nodeps_VERSION, Nodeps__MINIMUM_WP_VERSION ).'</strong> '.sprintf(__('Please <a href="%1$s">upgrade WordPress</a> to a current version, or <a href="%2$s">downgrade to version 2.4 of the Nodeps plugin</a>.', 'Nodeps'), 'https://codex.wordpress.org/Upgrading_WordPress', 'https://wordpress.org/extend/plugins/Nodeps/download/');

			Nodeps::bail_on_activation( $message );
		} else {
			add_option( 'Activated_Nodeps', true );
		}
	}
	...
	public static function plugin_deactivation( ) {
		...
		$Nodeps_cron_events = array(
			'Nodeps_schedule_cron_recheck',
			'Nodeps_scheduled_delete',
		);
		
		foreach ( $Nodeps_cron_events as $Nodeps_cron_event ) {
			$timestamp = wp_next_scheduled( $Nodeps_cron_event );
			
			if ( $timestamp ) {
				wp_unschedule_event( $timestamp, $Nodeps_cron_event );
			}
		}
	}
	 ...
	 * WP_DEBUG is true), but can be disabled via the Nodeps_debug_log filter.
	 *
	 * @param mixed $Nodeps_debug The data to log.
	 */
	public static function log( $Nodeps_debug ) {
		if ( apply_filters( 'Nodeps_debug_log', defined( 'WP_DEBUG' ) && WP_DEBUG && defined( 'WP_DEBUG_LOG' ) && WP_DEBUG_LOG && defined( 'Nodeps_DEBUG' ) && Nodeps_DEBUG ) ) {
			error_log( print_r( compact( 'Nodeps_debug' ), true ) );
		}
	}
	...
	public static function pre_check_pingback( $method ) {
		...
		if ( !empty( $args[1] ) ) {
			...
			$comment = Nodeps::auto_check_comment( $comment );

			if ( isset( $comment['Nodeps_result'] ) && 'true' == $comment['Nodeps_result'] ) {
	...
	/**
	 * Ensure that we are loading expected scalar values from Nodeps_as_submitted commentmeta.
	 *
	...
	public static function predefined_api_key() {
		...
		return apply_filters( 'Nodeps_predefined_api_key', false );
	}

	/**
	 * Controls the display of a privacy related notice underneath the comment form using the `Nodeps_comment_form_privacy_notice` option and filter respectively.
	...
	public static function display_comment_form_privacy_notice() {
		if ( 'display' !== apply_filters( 'Nodeps_comment_form_privacy_notice', get_option( 'Nodeps_comment_form_privacy_notice', 'hide' ) ) ) {
			return;
		}
		echo apply_filters(
			'Nodeps_comment_form_privacy_notice_markup',
			'<p class="Nodeps_comment_form_privacy_notice">' . sprintf(
				__( 'This site uses Nodeps to reduce spam. <a href="%s" target="_blank" rel="nofollow noopener">Learn how your comment data is processed</a>.', 'Nodeps' ),
				'https://Nodeps.com/privacy/'
			) . '</p>'
		);
	}
}
